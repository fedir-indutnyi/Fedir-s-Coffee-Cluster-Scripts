#!/usr/bin/env bash
# Script: find-ollama.sh
# Description: Scans the local network to find running Ollama instances
# Dependencies: bash (4+), iproute2 (ip), nmap, curl, grep, gawk (awk), util-linux (nl)
# Install on Ubuntu/Debian: apt install bash iproute2 nmap curl grep gawk util-linux

set -euo pipefail

# Check required commands
for cmd in ip nmap curl grep awk nl cut; do
...
interfaces=$(ip -4 -o addr show | grep -vE ' lo | docker | cni | flannel | kube | dummy | veth | br- ' | awk '{print $2, $4}') || true
...
selected_line=$(echo "${interfaces}" | grep -v '/32' | head -n1) || true
...
if ! nmap_output=$(nmap -p 11434 --open -n -oG - "${scan_subnet}" 2>/dev/null); then
  echo "❌ Error running nmap scan"
  exit 1
fi

mapfile -t hosts < <(echo "${nmap_output}" | grep "Ports: 11434/open" | awk '{print $2}')
